...
export default function BranchSell() {
  ...
  const getCartDetails = () => {
    ...
    const items = Object.entries(cart).map(([branchProductId, quantity]) => {
      ...
      return {
        ...
        unit: `${item.product.unitSize}${item.product.unitType}`,
        price,
        quantity: qty
      };
  ...
  const executeSale = async () => {
    ...
    try {
      ...
      setSaleComplete({ items, total, branchName: user?.name?.split(' User')[0] || 'HIMCREAM Branch' });
      setCart({});
      fetchInventory();
              ...
              <div className="flex items-center gap-3 w-full">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-xs text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded-md font-medium">
                      {item.product.unitSize} {item.product.unitType}
                    </span>
